...
export default function Layout() {
  ...
  return (
    <div className="relative h-screen w-screen bg-black overflow-hidden flex flex-col">
      ...
      {/* Navbar */}
      <nav className="absolute top-0 left-0 right-0 z-50 w-full px-6 py-6 border-b border-white/5 bg-black/10 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Link to="/" className="flex items-center gap-8">
            <img src="/logo.png" alt="Parul University GOA" className="h-12 w-auto object-contain" />
          </Link>
          <div className="flex items-center gap-4">
            {user ? (
              <>
                ...
                <button 
                  onClick={logout}
                  className="liquid-glass rounded-full px-6 py-2 text-white text-sm font-medium hover:bg-white/10 transition-colors cursor-pointer text-center"
                >
                  Logout
                </button>
              </>
              ...
              <>
                <Link to="/signup" className="text-white text-sm font-medium px-4 hover:text-white/80 transition-colors">Sign Up</Link>
                <Link to="/login" className="liquid-glass rounded-full px-6 py-2 text-white text-sm font-medium hover:bg-white/10 transition-colors cursor-pointer text-center">Login</Link>
              </>
            )}
          </div>
        </div>
